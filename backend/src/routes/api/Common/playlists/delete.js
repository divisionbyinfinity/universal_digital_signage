...
module.exports = async (req, res) => {
  ...
  try {
    ...
    await zipDirectory(`${process.env.CDN_PATH}playlist/${playlist.name}`,
                        `${process.env.CDN_PATH}recyclebin/playlist/${playlist.name}`,
                        playlist.name
    )
    
    // Delete CDN directory outside transaction
    await deleteDirectory(`${process.env.CDN_PATH}playlist/${playlist.name}`);
